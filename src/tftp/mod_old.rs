...
const RETRY_TIMEOUT: Duration = Duration { micros: 200*1000 };
    ...
    H: Handle,
{
    ...
    fn resend_data(&mut self, socket: &mut UdpSocket) -> net::Result<()> {
        if let Some(last_data) = &self.last_data {
            ...
            let payload = socket.send(data.buffer_len())?;
    ...
    fn send_ack(&mut self, socket: &mut UdpSocket, block: u16) -> net::Result<()> {
        ...
        let payload = socket.send(ack.buffer_len())?;
        let mut pkt = Packet::new_unchecked(payload);
        ack.emit(&mut pkt)
    }
}
...
fn send_error(
    ...
    let payload = socket.send(err.buffer_len())?;
    let mut pkt = Packet::new_unchecked(payload);
    err.emit(&mut pkt)
}
